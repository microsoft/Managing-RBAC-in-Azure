...
namespace RBAC
{
    public class UpdatePoliciesFromYaml
    {
        ...
        /*public int checkChanges(List<KeyVaultProperties> yamlVaults, List<KeyVaultProperties> vaultsRetrieved)
        {
            ...
            foreach (KeyVaultProperties kv in yamlVaults) 
            {
                if (!vaultsRetrieved.Contains(kv)) //if the vault AP have changed, then enter here 
                {
                    var oldVault = vaultsRetrieved.ToLookup(k => k.VaultName)[kv.VaultName];
                    if (oldVault.Count() != 0)
                    {
                        List<PrincipalPermissions> portalPolicies = oldVault.First().AccessPolicies;
                    foreach (PrincipalPermissions principalPermissions in kv.AccessPolicies)
                    {
                        if (!portalPolicies.Contains(principalPermissions)) //if old vault doesn't have this AP, then changed!
                        {
                            changes++;
                            ...
                            if (portalPolicy.Count() != 0) //if it is 0, do nothing because this pol was then added. if is 0, then it was updated
                            {
                                var portalPermissions = portalPolicy.First();
                                //then portalPermissions is the ap for old, principalPermissions is new ap 
                                ...
                                //create a new PP list and add to it with this shit
                        ...
                        {
                            portalPolicies.Remove(principalPermissions);
                        }
                        //so the list of OldAP you're left with is what was deletd


                    }
                    //then set oldVault acccess pols to the deleted ones and write the old vault to the yaml
                    

                    //if it exists
                    KeyVaultProperties deletedProperties = new KeyVaultProperties()
                    {
        ...
        public int checkChanges(List<KeyVaultProperties> yamlVaults, List<KeyVaultProperties> vaultsRetrieved)
        {
            ...
            List<KeyVaultProperties> deleted = new List<KeyVaultProperties>();
            foreach (KeyVaultProperties kv in yamlVaults)
            {
                ...
                if (!vaultsRetrieved.Contains(kv)) 
                {
                    ...
                    if (old.Count() != 0)
                    {
                        ...
                        foreach (PrincipalPermissions principalPermissions in kv.AccessPolicies)
                        {
                            if (!portalPolicies.Contains(principalPermissions))
                            {
                                ...
                                if (portalPolicy.Count() != 0)
                                {
                                    ...
                                    portalPermissions.PermissionsToKeys = portalPermissions.PermissionsToKeys.Select(k => k.ToLowerInvariant()).ToArray();
                                    portalPermissions.PermissionsToSecrets = portalPermissions.PermissionsToSecrets.Select(s => s.ToLowerInvariant()).ToArray();
                                    portalPermissions.PermissionsToCertificates = portalPermissions.PermissionsToCertificates.Select(c => c.ToLowerInvariant()).ToArray();

                                    string[] deletedKeys = portalPermissions.PermissionsToKeys.Except(principalPermissions.PermissionsToKeys.Select(k => k.ToLowerInvariant())).ToArray();
                                    string[] deletedSecrets = portalPermissions.PermissionsToSecrets.Except(principalPermissions.PermissionsToSecrets.Select(s => s.ToLowerInvariant())).ToArray();
                                    string[] deletedCertificates = portalPermissions.PermissionsToCertificates.Except(principalPermissions.PermissionsToCertificates.Select(c => c.ToLowerInvariant())).ToArray();
                        ...
                        deleted.Add(new KeyVaultProperties()
                        {
                            VaultName = oldVault.VaultName,
                            ResourceGroupName = oldVault.ResourceGroupName,
                            SubscriptionId = oldVault.SubscriptionId,
                        ...
                        for (int i = 0; i < oldVault.AccessPolicies.Count; i++)
                        {
                            var oldPol = oldVault.AccessPolicies[i];
                            var name = oldPol.DisplayName;
                            IEnumerable<PrincipalPermissions> curr;
                            if (oldPol.Type.ToLower() == "user" || oldPol.Type.ToLower() == "group")
                            {
                                curr = kv.AccessPolicies.ToLookup(pp => pp.Alias)[oldPol.Alias];
                            }
                            else
                            {
                                curr = kv.AccessPolicies.ToLookup(pp => pp.DisplayName)[name];
                            }
                            if (curr.Count() == 0)
                            {
                                changes++;
        ...
        {
            log.Info("Converting to Deleted YAML...");
            try
            {
                var serializer = new SerializerBuilder().Build();
                string yaml = serializer.Serialize(deleted);

                System.IO.File.WriteAllText(@"..\..\..\..\Config\DeleteYaml.yml", yaml);
                log.Info("Deleted YAML created!");
            }
            catch (Exception e)
            {
